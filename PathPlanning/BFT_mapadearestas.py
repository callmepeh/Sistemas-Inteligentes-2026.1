...
def build_bft_path(graph, start, goal,):
    ...
    while queue:
        ...
        if current == goal:
            ...
            return path[::-1]  # caminho invertido
        visited.add(current)
...
def plot_search_path(obstacles, polygons, vertices, edges, path):
    fig, ax = plt.subplots(figsize=(8, 8))
    ax.set_facecolor("#f7f7f7ff")
    ...
    ax.set_title("Grafo de Visibilidade com Caminho")
    ax.grid(True, linestyle="--", alpha=0.3)
    ax.legend()
...
if __name__ == "__main__":
    ...
    path = build_bft_path(graph, START, GOAL)
    plot_search_path(obstacles, polygons, vertices, edges, path)

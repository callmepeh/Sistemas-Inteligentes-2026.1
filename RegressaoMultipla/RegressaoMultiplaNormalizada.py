...
def calcular_medias_e_desvios(X):
    n = len(X)
    num_features = len(X[0]) - 1
    medias = []
    desvios = []
    for j in range(1, num_features + 1):
        valores = [x[j] for x in X]
        media = sum(valores) / n
        variancia = sum((valor - media) ** 2 for valor in valores) / n
        desvio = variancia ** 0.5 if variancia > 0 else 1.0
        medias.append(media)
        desvios.append(desvio)
    return medias, desvios


def normalizar_X(X, medias, desvios):
    return [[1.0] + [((x[j] - medias[j - 1]) / desvios[j - 1]) for j in range(1, len(x))] for x in X]
...
def _main():
    ...
    X_treino_raw, y_treino, X_teste_raw, y_teste = definir_conjunto_treino_e_teste(X, y, proporcao_treino=0.7)

    medias, desvios = calcular_medias_e_desvios(X_treino_raw)
    X_treino = normalizar_X(X_treino_raw, medias, desvios)
    X_teste = normalizar_X(X_teste_raw, medias, desvios)
    ...
    scatter_plot(X_treino_raw, y_treino, X_teste_raw, y_teste, coeficientes, mse, r2)
